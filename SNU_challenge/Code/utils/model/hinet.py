...
class SAM(nn.Module):
    ...
    def forward(self, x, x_img):
        ...
        x1 = self.conv1(x)
        img = self.conv2(x) + torch.index_select(x_img, 1, torch.tensor(0, device = x.device))
...
class HINet(nn.Module):
    ...
    def __init__(self, in_chn=2, out_chn=1, wf=32, depth=5, relu_slope=0.2, hin_position_left=0, hin_position_right=4):
    ...
    def forward(self, x):
        ...
        out_2 = self.last(x2)
        # input[0] is grappa
        out_2 = out_2 + torch.index_select(image, 1, torch.tensor(0, device = x.device))

        out_1 = out_1.squeeze(1)
        out_2 = out_2.squeeze(1)
        out_1 = self.unnorm(out_1)
        out_2 = self.unnorm(out_2)
        return out_2, out_1
